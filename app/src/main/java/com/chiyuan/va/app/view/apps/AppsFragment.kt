...
class AppsFragment : Fragment() {
    ...
    companion object {
        ...
        fun newAllUsersInstance(): AppsFragment = newInstance(ALL_USERS)
    }
    ...
    private fun stopApk(info: AppInfo) {
        try {
            MaterialDialog(requireContext()).show {
                ...
                positiveButton(R.string.done) {
                    try {
                        ChiyuanVACore.get().stopPackage(info.packageName, info.userId)
    ...
    fun installApk(source: String, targetUserId: Int = userID) {
        try {
            showLoading()
            viewModel.install(source, targetUserId)
        } catch (e: Exception) {
    ...
    private fun refreshApps() {
        if (userID == ALL_USERS) {
            viewModel.getInstalledAppsForAllUsers()
        } else {
    ...
    fun refreshAllApps() {
        if (this::viewModel.isInitialized) refreshApps()
    }

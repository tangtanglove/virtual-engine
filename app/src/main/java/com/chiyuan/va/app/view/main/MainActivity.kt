...
class MainActivity : LoadingActivity() {
    ...
    private fun initToolbarSubTitle() {
        try {
            viewBinding.toolbarLayout.toolbar.subtitle = "All clones"
        } catch (e: Exception) {
            Log.e(TAG, "Error in initToolbarSubTitle: ${e.message}")
        }
    }

    private fun initAppsGrid() {
        try {
            val existing = supportFragmentManager.findFragmentByTag("all_apps") as? AppsFragment
            appsFragment = existing ?: AppsFragment.newAllUsersInstance()
            if (existing == null) {
                supportFragmentManager.beginTransaction()
                        .replace(R.id.appsContainer, appsFragment, "all_apps")
                        .commitNow()
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error in initAppsGrid: ${e.message}")
        }
    }
    ...
    private fun initFab() {
        try {
            viewBinding.fab.setOnClickListener {
                try {
                    val users = ChiyuanVACore.get().users.sortedBy { it.id }
                    if (users.size <= 1) {
                        launchAppPicker(users.firstOrNull()?.id ?: 0)
                    } else {
                        val labels = users.map { user ->
                            AppManager.mRemarkSharedPreferences
                                    .getString("Remark${user.id}", "User ${user.id}")
                                    ?: "User ${user.id}"
                        }.toTypedArray()
                        androidx.appcompat.app.AlertDialog.Builder(this)
                                .setTitle("Select clone user")
                                .setItems(labels) { _, position -> launchAppPicker(users[position].id) }
    ...
    fun scanUser() {
        try {
            if (this::appsFragment.isInitialized) appsFragment.refreshAllApps()
        } catch (e: Exception) {
            Log.e(TAG, "Error in scanUser: ${e.message}")
        }
    }

    private fun launchAppPicker(userId: Int) {
                ...
                try {
                    if (it.resultCode == RESULT_OK) {
                        it.data?.let { data ->
                            ...
                            if (source != null) {
                                if (this::appsFragment.isInitialized) {
                                    appsFragment.installApk(source, userId)
                                }
                            }
                        }
                    }

...
public class LauncherActivity extends Activity {
    ...
    public static void launch(Intent intent, int userId) {
        try {
            ...
            splash.setClass(ChiyuanVACore.getContext(), LauncherActivity.class);
            ...
            ChiyuanVACore.getContext().startActivity(splash);
    ...
    private PackageInfo getPackageInfoWithFallback(String packageName, int userId) {
        try {
            ...
            return ChiyuanVACore.getBPackageManager().getPackageInfo(packageName, 0, userId);
        } catch (Exception e) {
            Slog.w(TAG, "Failed to get package info for " + packageName + " (attempt 1): " + e.getMessage());
            
            try {
                
                return ChiyuanVACore.getBPackageManager().getPackageInfo(packageName, 
                ...
                try {
                    ...
                    android.content.pm.ApplicationInfo appInfo = ChiyuanVACore.getBPackageManager()
    ...
    private void launchAppAsync(final Intent launchIntent, final int userId) {
        new Thread(() -> {
            try {
                ...
                ChiyuanVACore.getBActivityManager().startActivity(launchIntent, userId);

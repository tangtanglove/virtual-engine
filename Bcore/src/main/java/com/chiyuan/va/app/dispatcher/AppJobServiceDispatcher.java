...
public class AppJobServiceDispatcher {
    ...
    JobService getJobService(int jobId) {
        synchronized (mJobRecords) {
            ...
            try {
                JobRecord record = ChiyuanVACore.getBJobManager().queryJobRecord(ChiyuanVACore.getAppProcessName(), jobId);
                record.mJobService = ChiyuanVACore.currentActivityThread().createJobService(record.mServiceInfo);

package com.chiyuan.va.app.dispatcher;
...
import com.chiyuan.va.entity.ServiceRecord;
...
public class AppServiceDispatcher {
    ...
    private final Handler mHandler = ChiyuanVACore.get().getHandler();
    ...
    public int onStartCommand(Intent proxyIntent, int flags, int startId) {
        ...
        try {
            ...
            ChiyuanVACore.getBActivityManager().onStartCommand(proxyIntent, stubRecord.mUserId);
            return i;
        } catch (Throwable e) {
            e.printStackTrace();
        }
    ...
    public boolean onUnbind(Intent proxyIntent) {
        ...
        try {
            UnbindRecord unbindRecord = ChiyuanVACore.getBActivityManager().onServiceUnbind(proxyIntent, ChiyuanVACore.getUserId());
            ...
            if (destroy || record.decreaseConnectionCount(intent)) {
                ...
                if (destroy) {
                    service.onDestroy();
                    ChiyuanVACore.getBActivityManager().onServiceDestroy(proxyIntent, ChiyuanVACore.getUserId());
        ...
        if (record.getService() != null) {
            ...
            try {
                if (destroy) {
                    ...
                    ChiyuanVACore.getBActivityManager().onServiceDestroy(intent, ChiyuanVACore.getUserId());
                    mService.remove(new Intent.FilterComparison(intent));
                }
            } catch (Throwable e) {
                e.printStackTrace();
    ...
    private Service getOrCreateService(ProxyServiceRecord proxyServiceRecord) {
        ...
        Service service = ChiyuanVACore.currentActivityThread().createService(serviceInfo, token);
        if (service == null)
            return null;

...
import com.chiyuan.va.utils.compat.PackageParserCompat;
...
public class BPackageManagerService extends IBPackageManagerService.Stub implements ISystemService {
    public static final String TAG = "CVA_PackageMgr";
    ...
    public ApplicationInfo getApplicationInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, ChiyuanVACore.getHostPkg())) {
            try {
                return ChiyuanVACore.getPackageManager().getApplicationInfo(packageName, flags);
    ...
    public List<ResolveInfo> queryIntentServices(
        ...
        final String resolvedType = intent.resolveTypeIfNeeded(ChiyuanVACore.getContext().getContentResolver());
    ...
    public PackageInfo getPackageInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, ChiyuanVACore.getHostPkg())) {
            try {
                return ChiyuanVACore.getPackageManager().getPackageInfo(packageName, flags);
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
            }
            return null;
    ...
    private InstallResult installPackageAsUserLocked(String file, InstallOption option, int userId) {
        ...
        try {
            ...
            if (option.isFlag(InstallOption.FLAG_URI_FILE)) {
                ...
                InputStream inputStream = ChiyuanVACore.getContext().getContentResolver().openInputStream(Uri.parse(file));
            ...
            PackageInfo packageArchiveInfo = ChiyuanVACore.getPackageManager().getPackageArchiveInfo(apkFile.getAbsolutePath(), 0);
            ...
            if (packageName.equals(hostPackageName)) {
                return result.installError("Cannot clone ChiyuanVA app from within ChiyuanVA. This would create infinite recursion and is not allowed for security reasons.");
            }
            
            
            if (packageName.contains("chiyuanva") || packageName.contains("chiyuan") || 
                packageName.contains("vspace") || packageName.contains("virtual")) {
                
                Slog.w(TAG, "Installing potentially ChiyuanVA-related app: " + packageName + ". Proceed with caution.");
            }

            boolean support = AbiUtils.isSupport(apkFile);
            if (!support) {
                String msg = packageArchiveInfo.applicationInfo.loadLabel(ChiyuanVACore.getPackageManager()) + "[" + packageArchiveInfo.packageName + "]";
                return result.installError(packageArchiveInfo.packageName,
                        msg + "\n" + (ChiyuanVACore.is64Bit() ? "The box does not support 32-bit Application" : "The box does not support 64-bit Application"));
            }
            ...
            if (option.isFlag(InstallOption.FLAG_SYSTEM)) {
                aPackage.applicationInfo = ChiyuanVACore.getPackageManager().getPackageInfo(aPackage.packageName, 0).applicationInfo;
            }

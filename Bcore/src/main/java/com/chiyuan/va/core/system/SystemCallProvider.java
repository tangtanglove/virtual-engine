...
public class SystemCallProvider extends ContentProvider {
    public static final String TAG = "CVA_SysCallProvider";
    ...
    private boolean initSystem() {
        try {
            Slog.d(TAG, "Initializing ChiyuanVA system...");
            ChiyuanVASystem.getSystem().startup();
            Slog.d(TAG, "ChiyuanVA system initialized successfully");
            return true;
        } catch (Exception e) {
            Slog.e(TAG, "Failed to initialize ChiyuanVA system", e);
            return false;
        }
    }

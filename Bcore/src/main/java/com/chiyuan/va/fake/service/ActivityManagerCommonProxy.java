...
import com.chiyuan.va.fake.provider.FileProviderHandler;
...
import com.chiyuan.va.utils.compat.StartActivityCompat;
...
public class ActivityManagerCommonProxy {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            if (ComponentUtils.isRequestInstall(intent)) {
                ...
                if (file != null && file.exists()) {
                    try {
                        PackageInfo packageInfo = ChiyuanVACore.getPackageManager().getPackageArchiveInfo(file.getAbsolutePath(), 0);
                        if (packageInfo != null) {
                            String packageName = packageInfo.packageName;
                            String hostPackageName = ChiyuanVACore.getHostPkg();
                            if (packageName.equals(hostPackageName)) {
                                Slog.w(TAG, "Blocked attempt to install ChiyuanVA app from within ChiyuanVA: " + packageName);
                                
                                return 0;
                            }
                        }
                    } catch (Exception e) {
                        Slog.w(TAG, "Could not verify if this is ChiyuanVA app: " + e.getMessage());
                    }
                }
                
                if (ChiyuanVACore.get().requestInstallPackage(file, BActivityThread.getUserId())) {
                    return 0;
                }
            ...
            if (dataString != null && dataString.equals("package:" + BActivityThread.getAppPackageName())) {
                intent.setData(Uri.parse("package:" + ChiyuanVACore.getHostPkg()));
            }

            ResolveInfo resolveInfo = ChiyuanVACore.getBPackageManager().resolveActivity(
                    intent,
                    GET_META_DATA,
            ...
            if (resolveInfo == null) {
                ...
                resolveInfo = ChiyuanVACore.getBPackageManager().resolveActivity(
                        intent,
                        GET_META_DATA,
                        StartActivityCompat.getResolvedType(args),
                        BActivityThread.getUserId());
            ...
            ChiyuanVACore.getBActivityManager().startActivityAms(BActivityThread.getUserId(),
    ...
    public static class StartActivities extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            return ChiyuanVACore.getBActivityManager().startActivities(BActivityThread.getUserId(),
    ...
    public static class ActivityResumed extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ChiyuanVACore.getBActivityManager().onActivityResumed((IBinder) args[0]);
            return method.invoke(who, args);
        }
    }
    ...
    public static class ActivityDestroyed extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ChiyuanVACore.getBActivityManager().onActivityDestroyed((IBinder) args[0]);
            return method.invoke(who, args);
        }
    }
    ...
    public static class FinishActivity extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ChiyuanVACore.getBActivityManager().onFinishActivity((IBinder) args[0]);
            return method.invoke(who, args);
        }
    }
    ...
    public static class getCallingPackage extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            return ChiyuanVACore.getBActivityManager().getCallingPackage((IBinder) args[0], BActivityThread.getUserId());
        }
    }
    ...
    public static class getCallingActivity extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            return ChiyuanVACore.getBActivityManager().getCallingActivity((IBinder) args[0], BActivityThread.getUserId());
        }
    }
}
